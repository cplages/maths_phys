...
Vector3D Vector3D::mult_scal(float k) {
...
Vector3D Vector3D::composant_product(Vector3D *v) {
  float new_x = this->x * v->x;
  float new_y = this->y * v->y;
  float new_z = this->z * v->z;
  return Vector3D(new_x, new_y, new_z);
...
float Vector3D::dot_product(Vector3D *v) {
...
Vector3D Vector3D::proj( Vector3D *v) {
...
float Vector3D::triple_product(Vector3D *v,Vector3D *w) {
